%% Extensive Synthetic Outlier-Setting Sweep
%
...
%   6. Standard GPR
%
% This script runs five complete p_out sweeps:
%   1. N(0,10)      (variance 10)
%   2. N(0,50)      (variance 50)
%   3. U[0,15]
%   4. U[-15,0]
%   5. U[-10,10]
%
...
%   - A checkpoint is saved after each completed p_out value.
%   - Each complete outlier setting is saved and plotted immediately before
%     the script advances to the next setting.
...
% Root output directory. Each outlier setting receives its own subfolder.
outputRootDir = fullfile(userRoot, 'GPR_Synthetic_Extensive_5_Outlier_Settings');

if ~exist(outputRootDir, 'dir')
    mkdir(outputRootDir);
end

manifestFile = fullfile(outputRootDir, 'extensive_outlier_sweep_manifest.mat');

fprintf('\nAll case folders and the master manifest will be saved in:\n  %s\n', outputRootDir);
...
% Outlier cases
% ------------------------------------------------------------
% For the Gaussian cases, outlierScale is the standard deviation, so the
% requested variances 10 and 50 use sqrt(10) and sqrt(50), respectively.
% The requested bracketed N[-15,0] setting is interpreted as U[-15,0].
outlierCases = struct( ...
    'tag', { ...
        'Gaussian_Var10', ...
        'Gaussian_Var50', ...
    ...
    'model', { ...
        ...
        'negative_uniform', ...
        'uniform'}, ...
    'scale', {sqrt(10), sqrt(50), 15, 15, 10});

nOutlierCases = numel(outlierCases);
completedCaseMask = false(1, nOutlierCases);
completedCaseTimestamp = strings(1, nOutlierCases);
caseResultFiles = strings(1, nOutlierCases);
caseFigureBases = strings(1, nOutlierCases);

for caseIdx = 1:nOutlierCases

outlierCase = outlierCases(caseIdx);
outlierModel = outlierCase.model;
outlierScale = outlierCase.scale;
caseTag = outlierCase.tag;
caseLabel = outlierCase.label;

outputDir = fullfile(outputRootDir, sprintf('%02d_%s', caseIdx, caseTag));
if ~exist(outputDir, 'dir')
    mkdir(outputDir);
end

resultFile = fullfile(outputDir, sprintf('%s_results.mat', caseTag));

fprintf('\n\n############################################################\n');
fprintf('OUTLIER SETTING %d/%d: %s\n', caseIdx, nOutlierCases, caseTag);
fprintf('Model = %s | scale = %.8g\n', outlierModel, outlierScale);
fprintf('Case results will be saved in:\n  %s\n', outputDir);
fprintf('############################################################\n');


% The centered variance is recorded for reporting. The second moment
% E[e_out^2] is used only to configure the Oracle broad component; it is
% never passed to ASOR, GMM, Student-t, RCGPR, or Standard GPR.
...
opts.rcgp_epsilon = 0.05;                 % fixed across every p_out and outlier case
...
settings.caseIdx = caseIdx;
settings.caseTag = caseTag;
settings.caseLabel = caseLabel;
settings.nOutlierCases = nOutlierCases;
...
for pp = 1:nPout
    ...
    parfor mc = 1:nMC
        ...
        % ------------------------------------------------------------
        % Separate seeds ensure identical clean data, nominal noise, and
        % contamination masks across all five outlier settings.
        rng(200000 + 100*pp + mc, 'twister');
        ...
        rng(300000 + 100*pp + mc, 'twister');
        isOutlierEntry = rand(nTrain, p_y) < pOutNow;

        % Outlier amplitudes use a separate paired seed. Thus the two
        % Gaussian cases use the same standard-normal draws at different
        % scales, while the mask remains identical for every distribution.
        rng(400000 + 100*pp + mc, 'twister');
    ...
    save(resultFile, ...
        ...
        'caseIdx', 'caseTag', 'caseLabel', 'outlierCases', ...
...
ht = title(ax, caseLabel, 'Interpreter', 'latex');
...
figBase = fullfile(outputDir, sprintf('%s_pout_sweep_rmse_boxplot', caseTag));
figNow = gcf;
...
% Mark this entire distribution complete only after its MAT/CSV results and
% PNG/EPS/FIG figure files have all been attempted.
completedCaseMask(caseIdx) = true;
completedCaseTimestamp(caseIdx) = string(datestr(now, 'yyyy-mm-dd HH:MM:SS'));
caseResultFiles(caseIdx) = string(mainResultFile);
caseFigureBases(caseIdx) = string(figBase);

save(manifestFile, ...
    'outlierCases', 'completedCaseMask', 'completedCaseTimestamp', ...
    'caseResultFiles', 'caseFigureBases', 'pOutList', 'nMC', ...
    'nTrain', 'nTest', 'd_x_fixed', 'p_y_fixed', '-v7');

fprintf('\n============================================================\n');
fprintf('COMPLETED OUTLIER SETTING %d/%d: %s\n', ...
    caseIdx, nOutlierCases, caseTag);
fprintf('Results: %s\n', mainResultFile);
fprintf('Figure base: %s\n', figBase);
fprintf('Manifest updated immediately: %s\n', manifestFile);
fprintf('============================================================\n');
drawnow;

end  % for caseIdx = 1:nOutlierCases
