%% Real_Data_EnergyEfficiency_GaussianVar25
%
% Protocol:
%   1. Randomly select 100 training and 100 test observations.
%   2. Standardize inputs and outputs using training statistics only.
%   3. Add entry-wise Gaussian contamination only to standardized Ytrain.
%   4. Use outlier amplitude eta ~ N(0,25), so std(eta) = 5.
%   5. Evaluate every method against the uncontaminated standardized Ytest.
%   6. Repeat for p_out = 0,0.1,...,0.8 over 30 Monte Carlo trials.
%
% Methods:
...
%   6. Standard GPR
%
% The Energy Efficiency CSV is assumed to contain the two response columns
% as its final two numeric columns. Edit energyDataPath and gpmlPath below
% if the files are stored elsewhere.
...
outputDir = fullfile(scriptDir, 'energy_efficiency_gaussian_var25_results');
...
numOutputs = 2;
outputColumns = [];  % Empty means the final two columns are the outputs.

outlierStd = 5;
outlierVariance = outlierStd^2;  % 25

% Common initialization supplied to every non-oracle method.
sigmaInit = 0.5;

% The real dataset has no known measurement variance. The Oracle therefore
% uses the same nominal variance scale as the common initialization; its
% advantage is the exact artificial outlier labels and component assignment.
oracleInlierVariance = sigmaInit^2;
oracleOutlierVariance = oracleInlierVariance + outlierVariance;
...
fprintf('\n============================================================\n');
fprintf('Energy Efficiency real-data comparison\n');
fprintf('Gaussian outlier amplitude: N(0,25)\n');
fprintf('nTrain = %d | nTest = %d | nMC = %d\n', nTrain, nTest, nMC);
fprintf('p_out = %s\n', mat2str(pOutList));
fprintf('GMM covariance initialization: outlier = %.1f R0, nominal = %.1f R0\n', ...
...
for pp = 1:nPout
    ...
    fprintf('\nStarting p_out = %.2f (%d/%d)\n', pOutNow, pp, nPout);

    parfor mc = 1:nMC

        seedNow = 100000 + 100*pp + mc;
        rng(seedNow, 'twister');

        [Xtrain, Xtest, YcleanTrain, YcleanTest] = load_real_numeric_split_csv( ...
            energyDataPath, nTrain, nTest, seedNow, outputColumns, numOutputs);
        ...
        optsRun.rcgp_epsilon = opts.rcgp_epsilon;
        ...
        % Entry-wise independent N(0,25) contamination of Ytrain only.
        outlierNoise = outlierStd * randn(nTrain, p_y);
        ...
        fprintf(['p_out %.2f | MC %2d/%2d | Oracle %.4f | ASOR %.4f | ', ...
            'GMM %.4f | Student-t %.4f | RCGPR %.4f | Standard %.4f\n'], ...
            pOutNow, mc, nMC, RMSE_ORACLE(mc, pp), RMSE_ASOR(mc, pp), ...
            RMSE_GMM(mc, pp), RMSE_STUDENT(mc, pp), ...
            RMSE_RCGP(mc, pp), RMSE_STD(mc, pp));
    end

    fprintf('Completed p_out = %.2f in %.2f minutes.\n', ...
        pOutNow, toc(pOutTimer)/60);
end
...
% ========================================================================
results = struct();
results.dataset = 'Energy Efficiency';
results.dataPath = energyDataPath;
results.outlierDistribution = 'N(0,25)';
results.outlierStd = outlierStd;
...
results.totalWallTimeSeconds = toc(totalWallTimer);
results.options = opts;

dataCell = {RMSE_ORACLE, RMSE_ASOR, RMSE_GMM, ...
            RMSE_STUDENT, RMSE_RCGP, RMSE_STD};

resultsFile = fullfile(outputDir, ...
    'Energy_Efficiency_Gaussian_Var25_results.mat');
save(resultsFile, 'results', 'opts', 'methodNames', 'colors');

figureBase = fullfile(outputDir, ...
    'Energy_Efficiency_Gaussian_Var25_pout_sweep_boxplot');
